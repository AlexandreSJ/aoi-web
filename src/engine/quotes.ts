...
export function loadQuoteList(name: string): Promise<QuoteList> {
  if (name === "en") {
    return Promise.resolve({ name: "en", quotes: [...EN_QUOTES] });
  }
  return Promise.reject(new Error(`quote list "${name}" not found`));
}
